            ...
            <div class="email_content" style="margin-top:28px;">
                <form action="">
                    ...
                    <div class='captchaError' ref='captcha'></div>
                     <p :class="isAndroid?'c9 fz15 mt-15':'c9 fz15 mt-15 padd-5'" >请前往邮箱查看</p>
...
export default {
    ...
    mounted () {
        if(getUserInfoCookie()=='undefined'){
    ...
    methods:{
        ...
        async  goSuccess () {
            ...
            if(email && captcha && this.captchaData.result && this.emailHaveData.result == false
                ...
                await this.getEmailBoundSecurityAction(data);
                await info(that);
                async function info (that) {
                    let emailboundsecurityUrl = window.localStorage.getItem('emailboundsecurityUrl')
                    if(!that.emailBoundSecurityData.result){
                        that.flag=false;
                        that.$refs.email.style.borderTop='1px solid #f56c6c';
                        that.$refs.email.innerHTML=that.emailBoundSecurityData.msg;
                    }else{
                        that.$Toast('邮箱绑定成功')
                        that.$router.push({
                            path:emailboundsecurityUrl
                        })
                    }
                } 
            }
